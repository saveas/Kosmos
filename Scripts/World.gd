...
func spawn_enemy():
	...
	while not is_valid_spawn_pos:
		enemy.position=Vector2(rng.randi_range(camera_pos.x-300, camera_pos.x+300),rng.randi_range(camera_pos.y-600, camera_pos.y+600))
		if enemy.position.distance_to(camera_pos)>200:
